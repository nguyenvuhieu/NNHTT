...
const Data = () => {
  ...
  useEffect(() => {
    if (selectedDataGroup.id) {
      switch (selectedDataGroup.similarity_type) {
        case "COS_SIM":
          setSimilarityLabel(0);
          break;
        case "LEVEL":
          setSimilarityLabel(lev_array[0]); // Giá trị đầu tiên trong lev_array
          break;
        case "BOOL":
          setSimilarityLabel(false);
          break;
      }
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      const response = await api.post(`${HOST}/dataset`, newGroup); // Lưu phản hồi từ API
      fetchData(); // Refresh danh sách nhóm dữ liệu
      // Cập nhật selectedDataGroup với nhóm mới tạo
      if (response.status === 200) {
        // Kiểm tra xem tạo nhóm thành công
        setSelectedDataGroup({
          id: response.data._id, // Lấy _id của nhóm mới tạo từ phản hồi
          name: newGroupName,
          similarity_type: selectedSimilarity
        });
      }
      fetchData();
      setIsPopUpAddGroup(false);
  ...
  const handleDeleteGroup = async () => {
    ...
    try {
      ...
      // Gán selectedDocGroup cho nhóm đầu tiên nếu có
